...
(function(scope) {
  ...
  scope.layout = function() {
    ...
    function relayout() {
      ...
      /*var di; var dj; var groupSum;

      // find out if there is any 0-value groups:
      var emptyGroups = 0
      i = -1; while (++i < n) {
        di = groupIndex[i];
        groupSum = 0;
        j = -1; while (++j < n) {
            ...
            groupSum += matrix[indices[dj]][indices[di]];
        }
...
(function(scope) {
  ...
  scope.chord = function() {
    ...
    function chord(d, i) {
      ...
      // draw arrow if chord is wide enough
      var arrow = "";
      var deg = Math.PI / 180.0;
      var arrowWidth = 1.2 * deg;
      var arrowInnerRadius = t.r + 5;
      var arrowOuterRadius = arrowInnerRadius + 6.5;
     
      if(Math.abs((t.a1 - t.a0)) > (arrowWidth * 0.0)) {
        var centreOfArc = t.a0 + (t.a1 - t.a0) / 2;
        var arrowA0 = centreOfArc - (arrowWidth / 2);
        var arrowA1 = centreOfArc + (arrowWidth / 2);
        arrow =
            "M" + [arrowInnerRadius * Math.cos(arrowA0),     arrowInnerRadius * Math.sin(arrowA0)    ] + 
            "L" + [arrowInnerRadius * Math.cos(arrowA1),     arrowInnerRadius * Math.sin(arrowA1)    ] + 
            "L" + [arrowOuterRadius * Math.cos(centreOfArc), arrowOuterRadius * Math.sin(centreOfArc)] + 
            "Z";
      }
...
;
		...
		let changeSourceEvent = (item) => {
			...
			document.querySelector("#global-flow .timeline." + prefix).classList.add("active");

			if( document.querySelector("#global-flow .timeline." + prefix + " span.year input:checked") ) {
				year = document.querySelector("#global-flow .timeline." + prefix + " span.year input:checked").value;
			} else {
				year = "2015";
			}
		...
		document.querySelectorAll("#global-flow .timeline-info div").forEach((div) => {
			div.classList.remove("active");
		});

		document.querySelectorAll( "#global-flow .timeline-info ._" + year ).forEach((div) => {
	...
	function initDiagram1() {
		 ...
		 chart1 = Globalmigration.chart(migrationData, {
			...
			layout: {
				...
				colors: "f3c97d 007851 255ac4 7FA0D1 bacfd9 96c4b4 0e7991 2A3259 4e1192 9d4283 999999".split(' ').map(function(c) { return '#' + c; })
			}
		});
	 ...
	 function initDiagram3() {
		chart3 = Globalmigration.chart(binnenData, {
			...
			layout: {
				...
				threshold: 5000,
				labelThreshold: 5000,
				colors: "bacfd9 96c4b4 428DA1 175B75 2A3259 4E1192 AA3886 C48798 E48C71 D1AC90 F3C97D A3A96C 627945 007851 255AC4 7FA0D1".split(' ').map(function(c) { return '#' + c; })
			}
		});
